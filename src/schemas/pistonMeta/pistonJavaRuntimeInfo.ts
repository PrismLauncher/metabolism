...
const VERSION_PATTERN =
	/^(?<major>\d+)(?:\.(?<minor>\d+))?(?:\.(?<security>\d+))?(?:\.(?<build>\d+))?/;
const LEGACY_VERSION_PATTERN =
	/^(?<major>\d+)u(?<security>\d+)(?:b(?<build>\d+))?/;
...
export const PistonJavaRuntimeEntry = z.object({
	...
	version: z
		...
		.transform((version, context) => {
			...
			if (!match) {
				context.addIssue(
					`Got '${version.name}' - must match ${VERSION_PATTERN} or ${LEGACY_VERSION_PATTERN}`,
				);
				return z.NEVER;
			}
...
export const PistonJavaRuntimeInfo = z.record(
	z.string(),
	z.record(z.string(), z.array(PistonJavaRuntimeEntry)),
);
